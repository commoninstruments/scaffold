...
This repo is intentionally docs-first. It captures the shared baseline that is already emerging across recent active projects such as `materia`, `siteinspire`, `stow`, `designround`, `sorrel`, `kinecho`, `materialgraph`, plus the shared config repos `arc`, `lint`, `typescript-config`, and `turbo-config`.
